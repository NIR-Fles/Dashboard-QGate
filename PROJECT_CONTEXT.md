...
- **AI Models**: 
  ...
  - **PaddleOCR** for extracting 17-character Frame IDs, running in a background asynchronous thread during Step 1 to prevent blocking PLC loops.
...
- `backend/ocr_processor.py`: Wraps PaddleOCR. Used during Step 1 to read the 17-character Frame ID from a dynamically cropped region of the 'upper' camera frame.
- `backend/modbus_handler.py`: Runs a Modbus TCP server (Port 5020) in a background thread. Manages custom `TriggerDataBlock` and `SensorCoilDataBlock` to handle PLC/simulation triggers.
- `backend/state_manager.py`: Thread-safe dictionary managing the current state of the machine (engine status, PLC connection, bolt statuses, images). Has split fast/deferred paths for instant live view and background history saving.
...
Simulates physical sensor logic with edge-detection. To maintain a unified trigger pipeline, the coil combination writes the corresponding trigger code directly to Holding Register 1:
- `Coil 1`: Photosensor Entry
- `Coil 2`: Photosensor Middle
- `Coil 3`: Photosensor Exit
*Combination Matrix (updates HR 1):*
- `[ON, OFF, OFF]` = Unit Enter (HR 1 = 1)
- `[ON, ON, OFF]` = Capture Step 1 + Start Async OCR (HR 1 = 2)
- `[OFF, ON, ON]` = Capture Step 2 + Finalize (HR 1 = 3)
- `[OFF, OFF, ON]` = Unit Exit Armed (HR 1 = 4)
- `[OFF, OFF, OFF]` = Reset Sequence (HR 1 = 0)
...
- `1` = Unit Enter (Initializes new tracking, resets state to wipe leftover data from skipped exit sensors, auto-resets to `0` after 0.1s)
- `2` = Capture Step 1 (Trigger cameras, YOLO, and start background OCR, auto-resets to `0` after 0.1s)
- `3` = Capture Step 2 (Trigger cameras, YOLO, and finalize inspection, auto-resets to `0` after 0.1s)
...
*Security/Industrial Note: Input signals are protected by a global **Strict Sequence Validator**. The system strictly enforces the sequential progression (1 -> 2 -> 3 -> 4 -> 0) and ignores out-of-order jumps or bouncing. Auto-reset to 0 is disabled for 4 to support industrial ladder logic falling edges. Coils and direct writes both go through the exact same HR 1 pipeline.*
...
### Holding Register 4 (Output from Python to PLC)
Heartbeat signaling:
- `0` to `100` = PLC Heartbeat. An incrementing counter updated every 1.0 second. The PLC reads this to verify that the Python backend is alive and responsive.
...
- **Instant Dashboard & Asynchronous OCR**: To optimize latency, Step 1 uses `update_live_view` to instantly push downscaled images to the dashboard via WebSockets. Simultaneously, a background thread runs OCR to find the Frame ID, then sequentially saves the full-resolution history images (`save_history_image`) to `history_images/` using the corrected Frame ID, avoiding race conditions. The OCR text parser enforces exactly 17-character alphanumeric length and prioritizes patterns starting with `MH1` (Astra Honda Motor Indonesian VIN) to filter out casting codes.
